...
export async function POST(req: NextRequest) {
    ...
    try {
        ...
        if (Plek.Grootte == "K" || Plek.Grootte == "G") {
            //checkt als in body K, G, Klein of Groot is meegegeven
        } else if (Plek.Grootte == "Klein") {
            Plek.Grootte = "K";
        } else if (Plek.Grootte == "Groot") {
            Plek.Grootte = "G";
        } else {
            //checkt als groottes K of G zijn (klein of groot)
            return NextResponse.json({ error: "Ongeldig plek grootte" }, { status: 400 });
        }
