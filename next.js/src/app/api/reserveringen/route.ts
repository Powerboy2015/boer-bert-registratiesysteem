...
const allowedColumns = ["Voornaam", "Achternaam", "DatumAankomst", "DatumVertrek", "ReserveringsDatum", "PlekNummer"];
...
export async function GET(req: NextRequest) {
    ...
    const [rows] = await db.execute(`SELECT Voornaam, Achternaam, DatumAankomst, DatumVertrek, ReserveringsDatum, PlekNummer 
        FROM Reservingen INNER JOIN UserData ON UserData.ID = Reserveringen.UserData_ID
        ${whereSQLquery} ORDER BY ${sort} ${order} LIMIT ? OFFSET ?`, [...likeInput, limit, pagestart]);
...
export async function POST() {
    ...
    const [rows] = await db.execute(`INSERT INTO UserData (Voornaam, Achternaam, Telefoonnummer, WoonPlaats, Email)
        VALUES ()`);
